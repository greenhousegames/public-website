...
require.register("pages/games/blobclobber/reporting.js", function(exports, require, module) {
'use strict';
...
var Reporting = function (_FirebaseClient) {
  ...
  _createClass(Reporting, [{
    ...
    value: function _drawModesPlayed() {
      Promise.all([this.reporting.filter('modes', { mode: 'classic' }).sum('played').value(), this.reporting.filter('modes', { mode: 'survival' }).sum('played').value(), this.reporting.filter('modes', { mode: 'battle' }).sum('played').value()]).then(function (values) {
        var element = jQuery('#sum_played_chart');
        var data = new google.visualization.arrayToDataTable([['Mode', 'Times Played'], ['Classic', values[0] || 0], ['Survival', values[1] || 0], ['Battle', values[2] || 0]]);
    ...
    key: '_drawUserBreakdown',
    value: function _drawUserBreakdown() {
      Promise.all([this.reporting.filter('users-modes').max('classic-score').greater(0).count(), this.reporting.filter('users-modes').max('survival-score').greater(0).count(), this.reporting.filter('users-modes').max('battle-score').greater(0).count()]).then(function (values) {
        var element = jQuery('#user_breakdown_chart');
        var data = new google.visualization.arrayToDataTable([['Mode', 'Users'], ['Classic', values[0] || 0], ['Survival', values[1] || 0], ['Battle', values[2] || 0]]);
    ...
    key: '_drawDotsSmashed',
    value: function _drawDotsSmashed() {
      Promise.all([this.reporting.filter('modes', { mode: 'classic' }).sum('classic-circles').value(), this.reporting.filter('modes', { mode: 'survival' }).sum('survival-circles').value(), this.reporting.filter('modes', { mode: 'battle' }).sum('battle-circles').value()]).then(function (values) {
        var element = jQuery('#dots_smashed_chart');
        var data = new google.visualization.arrayToDataTable([['Mode', 'Dots Smashed'], ['Classic', values[0] || 0], ['Survival', values[1] || 0], ['Battle', values[2] || 0]]);
    ...
    value: function _drawMaxScores() {
      Promise.all([this.reporting.filter('modes', { mode: 'classic' }).max('classic-score').value(), this.reporting.filter('modes', { mode: 'survival' }).max('survival-score').value(), this.reporting.filter('modes', { mode: 'battle' }).max('battle-score').value()]).then(function (values) {
        var element = jQuery('#max_score_chart');
        var data = new google.visualization.arrayToDataTable([['Mode', 'Max Score'], ['Classic', values[0] || 0], ['Survival', values[1] || 0], ['Battle', values[2] || 0]]);
